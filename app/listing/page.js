...
import { SaveListing } from '@/lib/SavedListing'
    ...
    return (
        <div className='min-h-screen w-full'>
            ...
            <div className='min-h-screen pb-32 w-full p-5 flex gap-32 justify-center md:justify-evenly flex-wrap'>
                {data.map((item)=> (
                    <ListingsCard key={item.id} id={item.id} title={item.title} location={item.location} price={item.price} image={item.images[0]}/>
                ))}
            </div>
        </div>
    )
